...
def get_bucket_policy(bucket_name, account_number, partition):
    bucket_policy = {
        "Version": "2012-10-17",
        "Statement": [
            {
...
def create_bucket(unique_end, partition):
    ...
    try:
        ...
        s3.put_bucket_policy(
            Bucket=bucket_name, Policy=get_bucket_policy(bucket_name, account_number, partition)
        )
...
def update_custom_bucket_policy(bucket_name, account_number, partition):
...
def flow_log_activator(region_list, account_number, bucket_name, partition):
    """Function that turns on the VPC Flow Logs, for VPCs identifed without them"""
    for aws_region in region_list:
        ec2 = boto3.client("ec2", region_name=aws_region)
        logging.info(
            "Creating a list of VPCs without Flow Logs on in region " + aws_region + "."
        )
        try:
            VPCList: list = []
            FlowLogList: list = []
...
def check_cloudtrail(account_number, bucket_name):
    ...
    try:
        ...
        cloudtrail_status = cloudtrail.describe_trails(includeShadowTrails=True)
        if cloudtrail_status["trailList"] == []:
            ...
            cloudtrail_on = cloudtrail.start_logging(Name=cloudtrail_name)
...
def eks_logging(region_list):
    ...
    for aws_region in region_list:
        logging.info(
            "Turning on audit and authenticator logging for EKS clusters in region "
            + aws_region
            + "."
        )
...
def route_53_query_logs(region_list, account_number, bucket_name, partition):
    """Function to turn on Route 53 Query Logs for VPCs"""
    for aws_region in region_list:
        logging.info(
            "Turning on Route 53 Query Logging on for VPCs in region "
            + aws_region
            + "."
        )
        ec2 = boto3.client("ec2", region_name=aws_region)
        ...
        try:
            ...
            logging.info(
                "Route 53 Query Logging Created. Resource ID:" + r53_query_log_id
            )
            for vpc in r53_working_list:
                logging.info(
                    "Associating " + vpc + " with the created Route 53 Query Logging."
                )
                logging.info("AssociateResolverQueryLogConfig")
                activate_r53_logs = route53resolver.associate_resolver_query_log_config(
                    ResolverQueryLogConfigId=r53_query_log_id, ResourceId=vpc
                )
        except Exception as exception_handle:
            logging.error(exception_handle)
...
def s3_logs(region_list, unique_end):
    ...
    for aws_region in region_list:
        logging.info(
            "Checking for S3 Logging on for Buckets in region " + aws_region + "."
        )
        account_number = sts.get_caller_identity()["Account"]
        ...
        try:
            ...
            if S3List != []:
                ...
                for bucket in S3List:
                    if (
                        "aws-s3-log-collection-" + account_number + "-" + aws_region
                        not in str(bucket)
                    ):
...
def lb_logs(region_list, unique_end, partition):
    for aws_region in region_list:
        ...
        try:
            ...
            if ELBLogList != []:
                ...
                for elb in ELBLogList:
                    logging.info(
                        f"{elb[0]} does not have Load Balancer logging on. It will be turned on within this function."
                    )
                ...
                if aws_region == "us-east-1":
                    s3.create_bucket(
                        Bucket=f"aws-lb-log-collection-{account_number}-{aws_region}-{unique_end}"
                    )
                else:
                    s3.create_bucket(
                        Bucket=f"aws-lb-log-collection-{account_number}-{aws_region}-{unique_end}",
                ...
                logging.info(
                    f"Bucket aws-lb-log-collection-{account_number}-{aws_region}-{unique_end} Created."
                )
                logging.info("Setting lifecycle policy.")
                logging.info("PutBucketLifecycleConfiguration API Call")
                s3.put_bucket_lifecycle_configuration(
                    ...
                    "Statement": [
                        {
                            ...
                            "Principal": {"Service": "delivery.logs.amazonaws.com"},
                            "Action": "s3:GetBucketAcl",
                            "Resource": f"arn:{partition}:s3:::aws-lb-log-collection-{account_number}-{aws_region}-{unique_end}",
                        },
                        {
                            "Effect": "Allow",
                            "Principal": {"Service": "delivery.logs.amazonaws.com"},
                        ...
                        {
                            "Effect": "Allow",
                            "Principal": {"AWS": f"arn:{partition}:iam::{elb_account}:root"},
                            "Action": "s3:PutObject",
                            "Resource": f"arn:{partition}:s3:::aws-lb-log-collection-{account_number}-{aws_region}-{unique_end}/*",
                        },
                    ],
                ...
                if ELBv1LogList != []:
                    for elb in ELBv1LogList:
                        ...
                        elbv1client.modify_load_balancer_attributes(
                            LoadBalancerName=elb[0],
                            LoadBalancerAttributes={
                                "AccessLog": {
                                    "Enabled": True,
                                    "S3BucketName": f"aws-lb-log-collection-{account_number}-{aws_region}-{unique_end}",
                                    "EmitInterval": 5,
                ...
                if ELBv2LogList != []:
                    for elb in ELBv2LogList:
                        ...
                        elbv2client.modify_load_balancer_attributes(
                            LoadBalancerArn=elb[1],
                            Attributes=[
                                {"Key": "access_logs.s3.enabled", "Value": "true"},
                                {
                                    "Key": "access_logs.s3.bucket",
                                    "Value": f"aws-lb-log-collection-{account_number}-{aws_region}-{unique_end}",
                                },
                                {"Key": "access_logs.s3.prefix", "Value": elb[0]},
                            ],
                        )
                        logging.info("Logging Enabled for Load Balancer " + elb[0])
            else:
                logging.info(
                    "There are no Load Balancers to be set by Log Enabler in "
                    + aws_region
                )
        except Exception as exception_handle:
            logging.error(exception_handle)
...
def check_guardduty(region_list, account_number, bucket_name, partition):
    ...
    for aws_region in region_list:
        ...
        logging.info(
            "Checking for GuardDuty detector in the account "
            + account_number
            + ", region "
            + aws_region
        )
        try:
            ...
            if detectors["DetectorIds"] == []:
                ...
                logging.info(
                    "Created GuardDuty detector ID " + new_detector["DetectorId"]
                )
                ...
                if (
                    guardduty.get_detector(DetectorId=detector_id)["Status"]
                    == "DISABLED"
                ):
                    logging.info(
                ...
                gd_destinations = guardduty.list_publishing_destinations(
                    DetectorId=detector_id
                )["Destinations"]
                if gd_destinations == []:
                    logging.info(
                    ...
                    for dest in gd_destinations:
                        if dest["DestinationType"] == "S3":
                            ...
                            dest_info = guardduty.describe_publishing_destination(
                                DetectorId=detector_id, DestinationId=dest_id
                            )
...
def wafv2_logs(region_list, partition):
    account_number = sts.get_caller_identity()["Account"]
    bucket_arn = ""
    for aws_region in region_list:
        wafv2 = boto3.client("wafv2", region_name=aws_region)
        logging.info(
            "Checking for WAFv2 Logging in the account "
            + account_number
            + ", region "
            + aws_region
        )
        try:
            ...
            wafv2_regional_log_configs = wafv2.list_logging_configurations(
                Scope="REGIONAL"
            ...
            if aws_region == "us-east-1":
                ...
                logging.info("ListLoggingConfigurations API Call")
                wafv2_cf_log_configs = wafv2.list_logging_configurations(
                    Scope="CLOUDFRONT"
                )["LoggingConfigurations"]
            ...
            if WAFv2NoLogList != [] and bucket_arn == "":
                ...
                bucket_name = f"aws-waf-logs-ale-{account_number}-{unique_end}"
                logging.info("CreateBucket API Call")
                s3.create_bucket(Bucket=bucket_name)
            ...
            if WAFv2NoLogList != [] and bucket_arn != "":
                for arn in WAFv2NoLogList:
                    logging.info(
                        arn + " does not have logging turned on. Turning on logging."
                    )
