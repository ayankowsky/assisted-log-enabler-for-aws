...
# Find VPCs and check if VPC Flow Logs are on.
def dryrun_flow_log_activator(region_list, OrgAccountIdList, partition):
    logging.info("Creating a list of VPCs without Flow Logs on.")
    for org_account in OrgAccountIdList:
        for aws_region in region_list:
            sts = boto3.client("sts")
            RoleArn = (
                f"arn:{partition}:iam::{org_account}:role/Assisted_Log_Enabler_IAM_Role"
            )
            ...
            try:
                ...
                for no_logs in working_list:
                    logging.info(
                        no_logs
                        + " does not have VPC Flow logging on. This will not be turned on within the Dry Run option."
                    )
            except Exception as exception_handle:
                logging.error(exception_handle)


# List EKS Clusters for visibility.
def dryrun_eks_logging(region_list, OrgAccountIdList, partition):
    for org_account in OrgAccountIdList:
        for aws_region in region_list:
            logging.info(
                "Showing Amazon EKS clusters in AWS account "
                + org_account
                + ", in region "
                + aws_region
                + "."
            ...
            try:
                ...
                for cluster in eks_cluster_list:
                    logging.info(
                        "Please check if Audit and Authenticator logs are on for EKS Cluster "
                        + cluster
                    )
            except Exception as exception_handle:
                logging.error(exception_handle)
...
def dryrun_route_53_query_logs(region_list, OrgAccountIdList, partition):
    for org_account in OrgAccountIdList:
        for aws_region in region_list:
            logging.info(
                "Checking Route 53 Query Logging on in AWS Account "
                + org_account
                + " VPCs, in region "
                + aws_region
                + "."
            ...
            try:
                ...
                for no_query_logs in r53_working_list:
                    logging.info(
                        no_query_logs
                        + " does not have Route 53 Query logging on. Running Assisted Log Enabler for AWS will turn this on."
                    )
            except Exception as exception_handle:
                logging.error(exception_handle)
...
# Turn on S3 Logging.
def dryrun_s3_logs(region_list, OrgAccountIdList, partition):
    for org_account in OrgAccountIdList:
        for aws_region in region_list:
            logging.info(
            ...
            try:
                ...
                if S3List != []:
                    ...
                    for bucket in S3List:
                        if (
                            "aws-log-collection-" + org_account + "-" + aws_region
                            not in str(bucket)
                        ):
...
# Turn on LB Logging.
def dryrun_lb_logs(region_list, OrgAccountIdList, partition):
    for org_account in OrgAccountIdList:
        for aws_region in region_list:
            logging.info(
...
def dryrun_check_guardduty(region_list, OrgAccountIdList, partition):
        ...
        for aws_region in region_list:
            ...
            try:
                ...
                if detectors["DetectorIds"] == []:
                    ...
                    logging.info(
                        "Setting S3 Bucket as publishing destination for GuardDuty detector."
                    )
                else:
                    ...
                    if gd_destinations == []:
                        logging.info(
                            "Detector does not publish findings to a destination. Setting S3 Bucket as publishing destination for GuardDuty detector."
                        )
                    else:
...
def dryrun_wafv2_logs(region_list, OrgAccountIdList, partition):
    for org_account in OrgAccountIdList:
        for aws_region in region_list:
            logging.info(
            ...
            try:
                ...
                if WAFv2NoLogList != []:
                    for arn in WAFv2NoLogList:
                        logging.info(
                            arn
                            + " does not have logging turned on. Assisted Log Enabler would enable logging."
                        )
                else:
                    logging.info(
...
def lambda_handler(event, context):
    ...
    dryrun_flow_log_activator(region_list, OrgAccountIdList, partition)
    dryrun_eks_logging(region_list, OrgAccountIdList, partition)
    dryrun_route_53_query_logs(region_list, OrgAccountIdList, partition)
    dryrun_s3_logs(region_list, OrgAccountIdList, partition)
    dryrun_lb_logs(region_list, OrgAccountIdList, partition)
    dryrun_check_guardduty(region_list, OrgAccountIdList, partition)
    dryrun_wafv2_logs(region_list, OrgAccountIdList, partition)
    logging.info(
        "This is the end of the script. Please check the logs for the resources that would be turned on outside of the Dry Run option."
    )
