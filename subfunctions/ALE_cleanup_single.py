...
def r53_cleanup(region_list):
    """Function to clean up Route 53 Query Logging Resources"""
    logging.info(
        "Note: This script can take a while to finish, depending about how many Route 53 Query Log resources exist (about 60 seconds per Query Log resource) that were created by Assisted Log Enabler for AWS"
    )
    time.sleep(1)
    for aws_region in region_list:
        logging.info("---- LINE BREAK BETWEEN REGIONS ----")
        logging.info(
            "Cleaning up Route 53 Query Logging Resources in region " + aws_region + "."
        )
        ...
        try:
            ...
            )  # Collecting Arn of all Query Logs
            ...
            for r53_tag_info in QueryLogList:
                ...
                for value in r53_tags["Tags"]:
                    if (
                        value["Key"] == "Workflow"
                        and value["Value"] == "assisted-log-enabler"
                    ):
                        logging.info(
                            "The following Route 53 Query Logger was created by Assisted Log Enabler for AWS, and will be removed within this function: "
                            + r53_tag_info
                        )
                        QueryLogArnRemoveList.append(r53_tag_info)
            for Id in QueryLogArnRemoveList:
                logging.info(
                    "Gathering Resource ID for Route 53 Query Logging Resource to be removed."
                )
                logging.info("ListResolverQueryLogConfigs API Call")
                r53_resource_id = route53resolver.list_resolver_query_log_configs()[
                    "ResolverQueryLogConfigs"
                ][QueryLogArnRemoveList.index(Id)][
                    "Id"
                ]  # Collecting Resource ID for each Arn collected
                QueryLogIdRemoveList.append(r53_resource_id)
                logging.info(r53_resource_id + " added to removal list.")
            logging.info(
                "The following Resource IDs were created by Assisted Log Enabler for AWS, and will be removed within this function."
            )
            print(QueryLogIdRemoveList)
            for r53_remove in QueryLogIdRemoveList:
                logging.info(
                    "Gathering Query Log Config Associations for " + r53_remove
                )
                logging.info("ListResolverQueryLogConfigAssociations API Call")
                associated_vpcs = (
                    route53resolver.list_resolver_query_log_config_associations()
                )
                if (
                    associated_vpcs["TotalCount"] > 0
                    and associated_vpcs["ResolverQueryLogConfigAssociations"][0][
                        "ResolverQueryLogConfigId"
                    ]
                    == r53_remove
                ):
                    logging.info(
                        "The following Route 53 Query Logger is associated with a VPC, and will be removed within this function: "
                        + r53_remove
                    )
                    VPCRemovalList = []
                    ...
                    for vpc in VPCRemovalList:
                        logging.info(
                            "Removing "
                            + vpc
                            + " from Route 53 Query Logging configuration "
                            + r53_remove
                        )
                        logging.info("DisassociateResolverQueryLogConfig API Call")
                        removing_vpc = (
                            route53resolver.disassociate_resolver_query_log_config(
                                ResolverQueryLogConfigId=r53_remove, ResourceId=vpc
                            )
                        )
                        logging.info(vpc + " removed from " + r53_remove)
                        time.sleep(1)
                    logging.info(
                        "60 second pause to ensure disassociation of Amazon VPCs..."
                    )
                    time.sleep(60)
...
def cloudtrail_cleanup():
    ...
    try:
        logging.info(
            "Cleaning up CloudTrail Logs created by Assisted Log Enabler for AWS."
        )
        trail_list: list = []
        removal_list: list = []
        ...
        logging.info(
            "Listing CloudTrail trails created by Assisted Log Enabler for AWS."
        )
        print("Full trail list")
        print(trail_list)
        for removal_trail in trail_list:
            logging.info(
                "Checking tags for trails created by Assisted Log Enabler for AWS."
            )
            logging.info("ListTags API Call")
            trail_tags = cloudtrail.list_tags(ResourceIdList=[removal_trail])
            for tag_lists in trail_tags["ResourceTagList"]:
                for key_info in tag_lists["TagsList"]:
                    print(key_info)
                    if (
                        key_info["Key"] == "workflow"
                        and key_info["Value"] == "assisted-log-enabler"
                    ):
        ...
        for delete_trail in removal_list:
            ...
            cloudtrail.delete_trail(Name=delete_trail)
...
def vpcflow_cleanup(region_list):
    ...
    for aws_region in region_list:
        try:
            logging.info("---- LINE BREAK BETWEEN REGIONS ----")
            logging.info(
                "Cleaning up VPC Flow Logs created by Assisted Log Enabler for AWS in region "
                + aws_region
                + "."
            )
            removal_list: list = []
            ...
            vpc_flow_logs = ec2.describe_flow_logs(
                Filter=[
                    {"Name": "tag:workflow", "Values": ["assisted-log-enabler"]},
                ]
            )
            ...
            delete_logs = ec2.delete_flow_logs(FlowLogIds=removal_list)
            logging.info(
                "Deleted Flow Logs that were created by Assisted Log Enabler for AWS."
            )
            time.sleep(1)
        except Exception as exception_handle:
            logging.error(exception_handle)
...
def s3_cleanup(region_list):
    ...
    for aws_region in region_list:
        ...
        try:
            logging.info("---- LINE BREAK BETWEEN REGIONS ----")
            logging.info(
                "Cleaning up Bucket Logs created by Assisted Log Enabler for AWS in region "
                + aws_region
                + "."
            )
            removal_list: list = []
            ...
            for bucket in buckets["Buckets"]:
                s3region = s3.get_bucket_location(Bucket=bucket["Name"])[
                    "LocationConstraint"
                ]
            ...
            if removal_list != []:
                logging.info(
                    "List S3 Buckets with Logging enabled by by Assisted Log Enabler for AWS in "
                    + aws_region
                )
                print(removal_list)
                ...
                logging.info(
                    "Removed S3 Bucket Logging created by Assisted Log Enabler for AWS."
                )
                time.sleep(1)
            else:
                logging.info(
                    "There are no S3 Bucket set by Log Enabler in " + aws_region
                )
        except Exception as exception_handle:
            logging.error(exception_handle)
...
def lb_cleanup(region_list):
    """Function to clean up Load Balancer Logs"""
    logging.info(
        "Cleaning up Load Balancer Logs created by Assisted Log Enabler for AWS."
    )
    for aws_region in region_list:
        ...
        try:
            ...
            for lb in ELBList1["LoadBalancerDescriptions"]:
                ...
                lblog = elbv1client.describe_load_balancer_attributes(
                    LoadBalancerName=lb["LoadBalancerName"]
                )
                logging.info("Parsing out for ELB Access Logging")
                if lblog["LoadBalancerAttributes"]["AccessLog"]["Enabled"] == True:
                    if "aws-lb-log-collection-" in str(
                        lblog["LoadBalancerAttributes"]["AccessLog"]["S3BucketName"]
                    ):
            ...
            for lb in ELBList2["LoadBalancers"]:
                ...
                lblog = elbv2client.describe_load_balancer_attributes(
            ...
            if removal_list != []:
                logging.info(
                    "List Load Balancers with Logging enabled by by Assisted Log Enabler for AWS in "
                    + aws_region
                )
                print(removal_list)
                for elb in removal_list:
                    logging.info(
                        elb[0]
                        + " has Load Balancer logging on. It will be turned on within this function."
                    )
                if ELBv1LogList != []:
                ...
                if ELBv2LogList != []:
                    for elb in ELBv2LogList:
                        ...
                        logging.info("Logging Disabled for Load Balancer " + elb[0])
                logging.info(
                    "Removed Load Balancers Logging created by Assisted Log Enabler for AWS."
                )
                time.sleep(1)
            else:
                logging.info(
                    "There are no Load Balancers Logs set by Log Enabler in "
                    + aws_region
                )
        except Exception as exception_handle:
            logging.error(exception_handle)
...
def guardduty_cleanup(region_list):
    """ "Function to cleanup GuardDuty detectors"""
    logging.info(
        "Cleaning up GuardDuty detectors created by Assisted Log Enabler for AWS."
    )
    for aws_region in region_list:
        detector_list = []
        removal_list = []
        ...
        try:
            logging.info("---- LINE BREAK BETWEEN REGIONS ----")
            logging.info(
                "Cleaning up GuardDuty detectors created by Assisted Log Enabler for AWS in region "
                + aws_region
                + "."
            )
            ...
            if detector_list != []:
                ...
                print(detector_list)
                logging.info(
                    "Checking tags for GuardDuty detectors created by Assisted Log Enabler."
                )
                ...
                if removal_list != []:
                    logging.info(
                        "GuardDuty detectors created by Assisted Log Enabler to be deleted: "
                    )
                    print(removal_list)
                    ...
                    logging.info(
                        "There are no GuardDuty detectors created by Assisted Log Enabler in region "
                        + aws_region
                        + "."
                    )
            else:
                logging.info(
                    "No GuardDuty detectors enabled in region " + aws_region + "."
                )
...
def waf_cleanup(region_list):
    """Function to cleanup WAFv2 Logging Configurations"""
    logging.info(
        "Cleaning up WAFv2 logging previously enabled by Assisted Log Enabler."
    )
    ...
    for aws_region in region_list:
        ...
        try:
            logging.info(
                "Checking Web ACL logging configurations in region " + aws_region + "."
            )
            logging.info("ListLoggingConfigurations API Call")
            log_configs_regional = wafv2.list_logging_configurations(Scope="REGIONAL")[
            ...
            for acl in log_configs_regional:
                for destination in acl["LogDestinationConfigs"]:
                    if "aws-waf-logs-ale-" in destination:
                        logging.info(
                            "Deleting logging configuration for " + acl["ResourceArn"]
                        )
                        logging.info("DeleteLoggingConfiguration API Call")
                        wafv2.delete_logging_configuration(
                            ResourceArn=acl["ResourceArn"]
                        )

            if aws_region == "us-east-1":
                logging.info("Checking Global Web ACL logging configurations.")
                logging.info("ListLoggingConfigurations API Call")
                log_configs_cf = wafv2.list_logging_configurations(Scope="CLOUDFRONT")[
                    "LoggingConfigurations"
                ]
                for acl in log_configs_cf:
                    for destination in acl["LogDestinationConfigs"]:
                        if "aws-waf-logs-ale-" in destination:
                            logging.info(
                                "Deleting logging configuration for "
                                + acl["ResourceArn"]
                            )
                            logging.info("DeleteLoggingConfiguration API Call")
                            wafv2.delete_logging_configuration(
...
def run_vpcflow_cleanup():
    ...
    logging.info(
        "This is the end of the script. Please feel free to validate that logging resources have been cleaned up."
    )
