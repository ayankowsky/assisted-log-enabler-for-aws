...
def dryrun_check_cloudtrail():
    ...
    try:
        ...
        if cloudtrail_status["trailList"][0]["Name"] == "":
            logging.info(
                "There is no CloudTrail trail created within this account. Running Assisted Log Enabler for AWS will create the CloudTrail trail for this account."
            )
        else:
...
def dryrun_s3_logs(region_list, account_number):
    for aws_region in region_list:
        logging.info(
            "Turning on S3 Logging on for Buckets in region " + aws_region + "."
        )
