...
def assisted_log_enabler():
    ...
    parser = argparse.ArgumentParser(
        description="Assisted Log Enabler - Find resources that are not logging, and turn them on."
    )
    parser.add_argument(
        "--mode",
        help=" Choose the mode that you want to run Assisted Log Enabler in. Available modes: single_account, multi_account, cleanup, dryrun. WARNING: For multi_account, You must have the associated CloudFormation template deployed as a StackSet. See the README file for more details.",
    )
    parser.add_argument(
        "--bucket",
    ...
    parser.add_argument(
        ...
        help=" Specify a comma separated list of AWS account numbers to INCLUDE for multi_account mode.",
    )
    parser.add_argument(
        "--exclude_accounts",
        metavar="ACCOUNT_NUMBERS",
        help=" Specify a comma separated list of AWS account numbers to EXCLUDE for multi_account mode.",
    )

    function_parser_group = parser.add_argument_group(
        "Single & Multi Account Options",
        "Use these flags to choose which services you want to turn logging on for.",
    )
    function_parser_group.add_argument(
        "--all",
        action="store_true",
        help=" Turns on all of the log types within the Assisted Log Enabler for AWS (does not include GuardDuty).",
    )
    function_parser_group.add_argument(
        "--eks",
        action="store_true",
        help=" Turns on Amazon EKS audit & authenticator logs.",
    )
    function_parser_group.add_argument(
        "--vpcflow", action="store_true", help=" Turns on Amazon VPC Flow Logs."
    )
    function_parser_group.add_argument(
        "--r53querylogs",
        action="store_true",
        help=" Turns on Amazon Route 53 Resolver Query Logs.",
    )
    function_parser_group.add_argument(
        "--s3logs", action="store_true", help=" Turns on Amazon Bucket Logs."
    )
    function_parser_group.add_argument(
        "--lblogs", action="store_true", help=" Turns on Amazon Load Balancer Logs."
    )
    function_parser_group.add_argument(
        "--cloudtrail",
        action="store_true",
        help=" Turns on AWS CloudTrail. Only available in Single Account version.",
    )
    function_parser_group.add_argument(
        "--guardduty",
        action="store_true",
        help=" Turns on Amazon GuardDuty and exports findings to an S3 bucket. Will used specified bucket. WARNING: This creates a KMS Key to export findings.",
    )
    function_parser_group.add_argument(
        "--wafv2", action="store_true", help=" Turns on AWS WAFv2 Logs."
    )

    cleanup_parser_group = parser.add_argument_group(
        "Cleanup Options",
        "Use these flags to choose which resources you want to turn logging off for.",
    )
    cleanup_parser_group.add_argument(
        "--single_r53querylogs",
        action="store_true",
        help=" Removes Amazon Route 53 Resolver Query Log resources created by Assisted Log Enabler for AWS.",
    )
    cleanup_parser_group.add_argument(
        "--single_cloudtrail",
        action="store_true",
        help=" Removes AWS CloudTrail trails created by Assisted Log Enabler for AWS.",
    )
    cleanup_parser_group.add_argument(
        "--single_vpcflow",
        action="store_true",
        help=" Removes Amazon VPC Flow Log resources created by Assisted Log Enabler for AWS.",
    )
    cleanup_parser_group.add_argument(
        "--single_all",
        action="store_true",
        help=" Turns off all of the log types within the Assisted Log Enabler for AWS.",
    )
    cleanup_parser_group.add_argument(
        "--single_s3logs",
        action="store_true",
        help=" Removes Amazon Bucket Log resources created by Assisted Log Enabler for AWS.",
    )
    cleanup_parser_group.add_argument(
        "--single_lblogs",
        action="store_true",
        help=" Removes Amazon Load Balancer Log resources created by Assisted Log Enabler for AWS.",
    )
    cleanup_parser_group.add_argument(
        "--single_guardduty",
        action="store_true",
        help=" Removes Amazon GuardDuty detectors created by Assisted Log Enabler for AWS.",
    )
    cleanup_parser_group.add_argument(
        "--single_wafv2",
        action="store_true",
        help=" Removes AWS WAFv2 Logging Configurations created by Assisted Log Enabler for AWS.",
    )

    dryrun_parser_group = parser.add_argument_group(
        "Dry Run Options",
        "Use these flags to run Assisted Log Enabler for AWS in Dry Run mode.",
    )
    dryrun_parser_group.add_argument(
        "--single_account",
        action="store_true",
        help=" Runs Assisted Log Enabler for AWS in Dry Run mode for a single AWS account.",
    )
    dryrun_parser_group.add_argument(
        "--multi_account",
        action="store_true",
        help=" Runs Assisted Log Enabler for AWS in Dry Run mode for a multi-account AWS environment, using AWS Organizations.",
    )
    ...
    if args.mode == "single_account":
        ...
        if args.include_accounts:
            ...
            if all(len(a) == 12 for a in included_accounts_list):
                ...
                print(
                    "An invalid account number specified for --include_accounts. Account numbers are 12 digits long."
                )
        if args.exclude_accounts:
            ...
            if all(len(a) == 12 for a in excluded_accounts_list):
                ...
                sys.exit(
                    "An invalid account number was specified for --exclude_accounts. Account numbers are 12 digits long."
                )
        ...
        if args.eks:
            ...
            ALE_multi_account.run_vpc_flow_logs(
                bucket_name, included_accounts, excluded_accounts
            )
        elif args.r53querylogs:
            ...
            ALE_multi_account.run_guardduty(
                bucket_name, included_accounts, excluded_accounts
            )
        elif args.wafv2:
            ALE_multi_account.run_wafv2_logs(included_accounts, excluded_accounts)
        elif args.all:
            ALE_multi_account.lambda_handler(
                event, context, bucket_name, included_accounts, excluded_accounts
            )
        else:
            logging.info(
                "No valid option selected. Please run with -h to display valid options."
